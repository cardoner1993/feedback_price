...
EXPERIMENT_NAME = "torch_big_ner"
experiment_id = f"type_{EXPERIMENT_NAME}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
model_path = os.path.join('./output_torch', experiment_id)
...
text_data = pd.read_csv(os.path.join(LOAD_TOKENS_FROM, TRAIN_FILE))

if DEBUG:
    text_data = text_data.sample(n=100, random_state=1)

text_data = text_data[['id','text', 'entities']]
# pandas saves lists as string, we must convert back
text_data.entities = text_data.entities.apply(lambda x: literal_eval(x) )
...
text_data = text_data[['id','text', 'entities']]
train_dataset = text_data.loc[text_data['id'].isin(IDS[train_idx]),['text', 'entities']].reset_index(drop=True)
test_dataset = text_data.loc[text_data['id'].isin(IDS[valid_idx])].reset_index(drop=True)

print("FULL Dataset: {}".format(text_data.shape))
...
if not os.path.isfile(os.path.join(HF_HOME, 'pytorch_model.bin')):
    ...
    model = AutoModelForTokenClassification.from_pretrained(os.path.join(HF_HOME, 'pytorch_model.bin'), config=config_model)
...
def train(epoch):
    tr_loss, tr_accuracy, tr_f1score = 0, 0, 0
    ...
    for idx, batch in tqdm(enumerate(training_loader), total=len(training_loader)):
        ...
        if idx % 10 == 0:
            ...
            print(f"Training loss after {idx:04d} training steps: {loss_step}", flush=True)
        ...
        tmp_tr_f1score = f1_score(labels.cpu().numpy(), predictions.cpu().numpy(), average='macro')
    ...
    print(f"Training f1-macro epoch: {tr_f1score}")
...
if not LOAD_MODEL_FROM:
    ...
    torch.save(model.state_dict(), os.path.join(model_path, f'bigbird_v{VER}.pt'))
else:
